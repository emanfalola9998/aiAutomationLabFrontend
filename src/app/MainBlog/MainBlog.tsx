...
const MainBlog = () => {
  ...
  const commentsPerBlog = useSelector(
    (state: RootState) => state.ai.commentsByBlog
  );

  const filteredBlogs = filtration(allBlogs, {
    isAutomation,
    isImpactful,
    isFuture,
  });
  ...
  const filteredBlogsBySearch = allBlogs.filter(
    (blog) =>
      blog.tags.toLowerCase().includes(searchTerm) ||
      blog.author.toLowerCase() == searchTerm ||
      ...
      <div className="flex flex-row justify-center items-start text-2xl  ">
        ...
        >
          {!anyFilterActive && searchTerm && (
            <h1>
              {" "}
              <span className="text-blue-500">You searched for</span> "
              {searchTerm}"
            </h1>
          )}

          <AnimatePresence>
            {(!anyFilterActive && searchActive
              ...
              >
                <Link
                  href={`/blog/${blog.id}`}
                  className=" p-10  animate-fadeInList flex flex-row justify-between items-center border-b border-gray-300/40 md:p-5 md:my-2"
                >
                  <div className="flex flex-col gap-4 p-5">
                    ...
                    <div className="flex flex-row text-gray-600 gap-4 items-center">
                      <div className="text-sm">
                        {formatDateWithConditionalYear(blog.datePublished)}
                      </div>
        ...
        <div className="text-xl sticky top-0 h-screen overflow-hidden hidden md:flex md:flex-col md:gap-4 mt-8 m-10  border-l border-gray-300/40 pl-10">
          <TrendingTopics />
